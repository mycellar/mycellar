 ...
 */
@Repository
public class JpaRegionRepository extends
        JpaEntitySearchFormRepository<Region, RegionOrderEnum, RegionOrder> implements
        RegionRepository {
     ...
     */
    @Override
    public Region find(Country country, String name) {
        ...
        try {
            return getEntityManager().createQuery(
                    query.select(root).where(criteriaBuilder.equal(root.get("country"), country),
     ...
     */
    @Override
    protected Expression<?> getOrderByPath(Root<Region> root, RegionOrderEnum order,
            JoinType joinType) {
        switch (order) {
        case COUNTRY_NAME:
        ...
        default:
            throw new IllegalStateException("Unknown " + RegionOrderEnum.class.getSimpleName()
                    + " value [" + order + "].");
        }
    }
     ...
     */
    @Override
    protected Order[] getOrderByForAllWithCount(Root<Region> root, CriteriaBuilder criteriaBuilder,
            JoinType joinType) {
     ...
     */
    @Override
    protected From<?, User> getOwnerJoin(Root<Region> root, JoinType joinType) {
     ...
     */
    @Override
    protected From<?, Region> getRegionJoin(Root<Region> root, JoinType joinType) {
        return root;
    }
     ...
     */
    @Override
    protected From<?, Producer> getProducerJoin(Root<Region> root, JoinType joinType) {
     ...
     */
    @Override
    protected From<?, Format> getFormatJoin(Root<Region> root, JoinType joinType) {
     ...
     */
    @Override
    protected From<?, Country> getCountryJoin(Root<Region> root, JoinType joinType) {
        return JoinHelper.getJoin(root, "country", joinType);
    }
     ...
     */
    @Override
    protected From<?, Wine> getWineJoin(Root<Region> root, JoinType joinType) {
     ...
     */
    @Override
    protected From<?, Cellar> getCellarJoin(Root<Region> root, JoinType joinType) {
     ...
     */
    @Override
    protected From<?, Appellation> getAppellationJoin(Root<Region> root, JoinType joinType) {
        return JoinHelper.getJoinSet(root, "appellations", joinType);
    }
     ...
     */
    @Override
    protected From<?, CellarShare> getSharesJoin(Root<Region> root, JoinType joinType) {
     ...
     */
    @Override
    protected From<?, Stock> getStockJoin(Root<Region> root, JoinType joinType) {
