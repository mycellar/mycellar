 ...
 */
@Service
public class AppellationMapper extends
        AbstractMapper<fr.peralta.mycellar.interfaces.facades.wine.Appellation, Appellation> {

    @Override
    public Appellation map(fr.peralta.mycellar.interfaces.facades.wine.Appellation dto) {
        return new Appellation(dto.getName(), getMapperServiceFacade().map(dto.getRegion(),
                Region.class), getMapperServiceFacade().map(dto.getMap(), Map.class),
                dto.getDescription());
    }
